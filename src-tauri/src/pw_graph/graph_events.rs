...
impl FromStr for PortDirection {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s.to_lowercase().as_str() {
            // Probably don't need to have it be lowercase
...
pub(crate) struct NodeEvent {
    object_id: u32,
    application_name: Option<String>, // Optional because the dummy driver doesn't have an Application Name
    node_name: String,
}
...
pub(crate) fn get_event(pw_event: &GlobalObject<ForeignDict>) -> Result<GraphEvent, ()> {
    match &pw_event.type_ {
        pipewire::types::ObjectType::Node => parse_node_event(pw_event),
        pipewire::types::ObjectType::Port => parse_port_event(pw_event),
        pipewire::types::ObjectType::Link => parse_wire_event(pw_event),
...
fn parse_port_event(pw_event: &GlobalObject<ForeignDict>) -> Result<GraphEvent, ()> {
    let properties = pw_event.props.as_ref().unwrap();
    Ok(GraphEvent::Port(PortEvent {
...
fn parse_wire_event(pw_event: &GlobalObject<ForeignDict>) -> Result<GraphEvent, ()> {
    let properties = pw_event.props.as_ref().unwrap();
    Ok(GraphEvent::Wire(WireEvent {
        object_id: pw_event.id,
...
fn parse_node_event(pw_event: &GlobalObject<ForeignDict>) -> Result<GraphEvent, ()> {
    let properties = pw_event.props.as_ref().unwrap();
    Ok(GraphEvent::Node(NodeEvent {
        object_id: pw_event.id,
